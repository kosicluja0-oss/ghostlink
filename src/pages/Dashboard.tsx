            ...
            <main className="p-4 lg:p-6">
              ...
              <section className="mb-5">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                  <StatCard label="Total Clicks" value={displayStats.totalClicks.toLocaleString()} icon={MousePointer} trend={clicksTrend ?? undefined} accentColor="primary" compact tooltip="Total number of times someone clicked on your tracking link. Every visit counts — even repeat visitors." isActive={activeMetric === 'clicks'} onClick={() => handleMetricChange('clicks')} />
                  <StatCard label="Total Leads" value={displayStats.totalLeads.toLocaleString()} icon={Users} trend={leadsTrend ?? undefined} isLocked={isFreeTier} accentColor="warning" compact tooltip="People who took a desired action after clicking — like signing up, submitting a form, or opting in. A lead means someone showed real interest." isActive={activeMetric === 'leads'} onClick={() => handleMetricChange('leads')} />
                  <StatCard label="Total Sales" value={displayStats.totalSales.toLocaleString()} icon={TrendingUp} trend={salesTrend ?? undefined} isLocked={isFreeTier} accentColor="success" compact tooltip="Confirmed purchases made through your links. Each sale means someone bought the product or service you're promoting." isActive={activeMetric === 'sales'} onClick={() => handleMetricChange('sales')} />
                  <StatCard label="Revenue" value={formatCurrency(displayStats.totalEarnings)} icon={DollarSign} trend={earningsTrend ?? undefined} isLocked={isFreeTier} accentColor="success" compact tooltip="Total money earned from all your sales. This is the sum of commissions or product prices generated through your links." isActive={activeMetric === 'revenue'} onClick={() => handleMetricChange('revenue')} />
